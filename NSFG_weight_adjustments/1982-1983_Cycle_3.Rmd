---
title: "Cycle 3 (1982) NSFG Weight Adjustment"
author: "Joshua Born"
output:
  html_document:
    code_folding: hide
...
data_fem <- select(
  ...
  left_join(
    data_preg |>
      group_by(CASEID) |>
      summarize(
        abortions_1974_1979 = sum(OUTCOME == 'INDUCED ABORTION' & Year %in% 1974:1979),
        ...
        in_1974_abortion = any(OUTCOME == 'INDUCED ABORTION' & Year == 1974),
        in_1975_abortion = any(OUTCOME == 'INDUCED ABORTION' & Year == 1975),
        in_1976_abortion = any(OUTCOME == 'INDUCED ABORTION' & Year == 1976),
        in_1977_abortion = any(OUTCOME == 'INDUCED ABORTION' & Year == 1977),
        in_1978_abortion = any(OUTCOME == 'INDUCED ABORTION' & Year == 1978),
        in_1979_abortion = any(OUTCOME == 'INDUCED ABORTION' & Year == 1979),
  ...
  mutate(
    abortions_1974_1979 = coalesce(abortions_1974_1979, 0),
    abortions_1974_1979_multiplier = if_else(
      abortions_1974_1979 >= 1,
      abortions_1974_1979,
      1
    ),
    across(
      starts_with('in_'),
      function(x) { coalesce(x, FALSE) }
    ),
    across(
      starts_with('birth_'),
      function(x) {
...
```{r check-counts-of-pregnancies, dependson="data-preparation"}
data_preg |>
  filter(Year %in% 1974:1979, OUTCOME %in% c('INDUCED ABORTION', 'LIVE BIRTH')) |>
  group_by(OUTCOME, age_group6) |>
  count() |>
  ungroup(age_group6) |>
  gt()
...
guttmacher_APC_year <- guttmacher_APC_national |>
  select(year, abortionstotal) |>
  filter(year %in% 1974:1979) |>
  mutate(
    Variable = paste0('in_', year, '_abortion'),
    `TRUE` = abortionstotal,
    `FALSE` = population_total - abortionstotal,
    .keep = 'unused'
  ) |>
  pivot_longer(
    c('TRUE', 'FALSE')
...
extras[[1]] <- c(
  ~abortions_1974_1979_multiplier,
  ~abortion_1974plus_age
)
...
post_fem <- rake2(
  ...
  extras = extras,
  control=list(maxit=150, epsilon=1, verbose=TRUE)
)
```
...
after_fem |>
  ...
  summarize(
    Count = n(),
    Est = survey_total(abortions_1974_1979_multiplier)
  ) |>
  full_join(
...
check_poststratifications(post_stratification_targets[[8]])
```
