...
data_fem <- select(
  ...
  mutate(
    ...
    abortions_1974_1979_multiplier = max(abortions_1974_1979, 1),
    across(
      starts_with('birth_'),
      function(x) {
        factor(
...
### Number of Respondents with Multiple Abortions in Analysis Period

```{r}
data_preg |>
  filter(OUTCOME == 'INDUCED ABORTION') |>
  group_by(CASEID) |>
  summarize(`Abortions in Analysis Period` = n(), .groups = 'drop') |>
  group_by(`Abortions in Analysis Period`) |>
  count() |>
  ungroup() |>
  gt()
```
