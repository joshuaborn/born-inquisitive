...
data_fem <- select(
  ...
  left_join(
    data_preg |>
      group_by(CASEID) |>
      summarize(
        abortion_1986_1994_age = case_when(
          any(OUTCOME == 'INDUCED ABORTION' & Year %in% 1986:1994 & AgeAtEndOfPregnancy <= 19) ~ 'Under 20 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year %in% 1986:1994 & AgeAtEndOfPregnancy >= 20 & AgeAtEndOfPregnancy <= 24) ~ '20-24 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year %in% 1986:1994 & AgeAtEndOfPregnancy >= 25 & AgeAtEndOfPregnancy <= 29) ~ '25-29 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year %in% 1986:1994 & AgeAtEndOfPregnancy >= 30 & AgeAtEndOfPregnancy <= 34) ~ '30-34 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year %in% 1986:1994 & AgeAtEndOfPregnancy >= 35 & AgeAtEndOfPregnancy <= 39) ~ '35-39 years',
          .default = 'Other'
        ),
        abortion_1986plus_age = case_when(
          any(OUTCOME == 'INDUCED ABORTION' & Year >= 1986 & AgeAtEndOfPregnancy <= 19) ~ 'Under 20 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year >= 1986 & AgeAtEndOfPregnancy >= 20 & AgeAtEndOfPregnancy <= 24) ~ '20-24 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year >= 1986 & AgeAtEndOfPregnancy >= 25 & AgeAtEndOfPregnancy <= 29) ~ '25-29 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year >= 1986 & AgeAtEndOfPregnancy >= 30 & AgeAtEndOfPregnancy <= 34) ~ '30-34 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year >= 1986 & AgeAtEndOfPregnancy >= 35 & AgeAtEndOfPregnancy <= 39) ~ '35-39 years',
          .default = 'Other'
        ),
  ...
  mutate(
    ...
    abortion_1986plus_age = if_else(
      abortion_1986plus_age != abortion_1986_1994_age & abortion_1986_1994_age != 'Other',
      abortion_1986_1994_age,
      abortion_1986plus_age
    )
  )
...
guttmacher_APC_age <- guttmacher_APC_national |>
  ...
  pivot_longer(
    cols = everything(),
    names_to = 'abortion_1986_1994_age',
    values_to = 'Freq'
  ) |>
  select(abortion_1986_1994_age, Freq) %>%
  bind_rows(summarize(
    .,
    abortion_1986_1994_age = 'Other',
    Freq = population_total - sum(Freq)
  )) |>
  mutate(
    abortion_1986_1994_age = factor(
      abortion_1986_1994_age,
      levels = c(
        'Under 20 years',
        '20-24 years',
...
weight_adj_strata[[length(post_stratification_targets)]] <- ~abortion_1986plus_age
...
entropy_extreme_after |>
  ...
  gt() |>
  fmt_number(c(contains('WGT'), 'POST_WT'), decimal = 0)
```
...
after_fem |>
  ...
  full_join(
    ...
    by = join_by(age_granular_ethnicity_race)
  ) |>
  mutate(
    Diff = Est - Freq
  ) |>
...
after_fem |>
  ...
  full_join(
    ...
    by = join_by(age_granular_ethnicity_race_marital_history)
  ) |>
  mutate(
    Diff = Est - Freq
  ) |>
...
after_fem |>
  ...
  full_join(
    ...
    by = join_by(age_granular_ethnicity_race_parity)
  ) |>
  mutate(
    Diff = Est - Freq
  ) |>
...
after_fem |>
  group_by(birth_1992_age) |>
  summarize(
    Count = n(),
    Est = survey_total(one)
  ) |>
  full_join(
    post_stratification_targets[[10]],
    by = join_by(birth_1992_age)
  ) |>
  mutate(
    Diff = Est - Freq
  ) |>
...
after_fem |>
  group_by(birth_1993_age) |>
  summarize(
    Count = n(),
    Est = survey_total(one)
  ) |>
  full_join(
    post_stratification_targets[[11]],
    by = join_by(birth_1993_age)
  ) |>
  mutate(
    Diff = Est - Freq
  ) |>
...
after_fem |>
  group_by(birth_1994_age) |>
  summarize(
    Count = n(),
    Est = survey_total(one)
  ) |>
  full_join(
    post_stratification_targets[[12]],
    by = join_by(birth_1994_age)
  ) |>
  mutate(
    Diff = Est - Freq
  ) |>
...
after_fem |>
  group_by(abortion_1986_1994_age) |>
  summarize(
    Count = n(),
    Est = survey_total(one)
  ) |>
  full_join(
    post_stratification_targets[[13]],
    by = join_by(abortion_1986_1994_age)
  ) |>
  mutate(
    Diff = Est - Freq
  ) |>
...
before_fem |>
  group_by(abortion_1986plus_age) |>
  summarize(
    Count = n(),
    Before = survey_total(one)
  ) |>
  full_join(
    ...
    by = join_by(abortion_1986plus_age)
  ) |>
  mutate(
    Factor = After / Before
  ) |>
