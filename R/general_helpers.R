...
style_totals_and_percentages <- function(dt) {
  ...
  set_left_padding(everywhere, c(2, 4), 12) |>
  set_tb_padding(1:2, everywhere, 0)
}
...
combine_huxtables_vertically <- function(label1, ht1, label2, ht2) {
  if (!all.equal(ht1[header_rows(ht1),], ht2[header_rows(ht2),])) {
    ...
    body_start_index1 <- body_rows1[1]
    body_start_index2 <- body_rows1[-1] + 1
    ...
    insert_column(ht, label_column) |>
      ...
      theme_article() |>
      set_bold(everywhere, old_header_columns, FALSE) |>
      set_left_padding(everywhere, header_column_index, 4) |>
      set_right_padding(everywhere, final(1), 4) |>
      set_right_padding(everywhere, header_column_index, 12) |>
      set_tb_padding(everywhere, header_column_index, 0) |>
      set_bottom_border(body_rows1[-1], everywhere)
  }
}
