...
Understanding how contraception is used is necessary for understanding the phenomenon of contraceptive failure and the related phenomenon of unintended pregnancy. The purpose of this report is to describe the current state of contraception use in the United States, such as the number of people using and not using contraception, the contraceptive methods being used, and motives for using or discontinuing the various contraceptive methods. This is intended to be a preliminary report to inform subsequent analyses that pose and answer more specific questions and, as such, is designed for breadth rather than depth.
...
Many variables suppressed or modified in the public use data files are available through an application process via a NCHS Research Data Center. However, this report is based entirely on the NSFG public use data files. Whenever analysis has constrained in some way by the disclosure risk reduction of the public use data files, this is noted in the report.
...
Prior works based on the NSFG have reported on the same subject matter of this report. [@kavanaugh_contraceptive_2018] Indeed, personnel associated with the NCHS themselves published reports based on the NSFG in journals [@piccinino_trends_1998], National Health Statistics Reports [@jones_current_2012; @daniels_contraceptive_2013; @copen_condom_2017], and NCHS Data Briefs [@daniels_current_2020]. This report differs from prior works in several ways.

Unlike many prior works, this report is based on the latest 2017-2019 edition of the NSFG. While several academic papers on more specific topics based on the 2017-2019 data have already been published in the literature, only a single NCHS Data Brief currently covers contraceptive use as reported in the 2017-2019 NSFG more generally. [@daniels_current_2020]

Many prior works based on the NSFG focus exclusively on contraceptive use among females living in the United States. [@piccinino_trends_1998; @jones_current_2012; @daniels_contraceptive_2013; @daniels_current_2020] This report examines contraceptive use of both females and males and analyzes female and male contraceptive use alongside each other, wherever data is available in both NSFG data files.

* Last intercourse instead of month of interview
* Multiple methods
* Minimal covariate analysis
* No trend analysis
* Topics reported upon separately, e.g., current method choice vs. contraceptive methods ever used, combined in this report
