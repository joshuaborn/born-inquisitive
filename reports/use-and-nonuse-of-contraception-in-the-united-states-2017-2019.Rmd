...
Combining the estimates from Table \@ref(tab:used-last-3m-at-all) and Table \@ref(tab:reason-for-nonuse), an overall contraceptive use rate can be be calculated with the number of persons using contraception in the numerator and the number persons at risk for unintended pregnancy in the denominator.
...
Among those in this subdomain, a substantial minority do not have any intent to become pregnant and so are at increased risk for unintended pregnancy due to lack of contraception use. (Table \@ref(tab:pregnancy-intent-status))
...
The average[^mean] age at which females living in households in the United States have sterilizing operations performed is approximately `r this$mean` years, with 95\% confidence interval `r this$ci` years.
...
```
 ...
 The number of females who have had sterilizing operations such as tubal ligation increases with age group until the early forties, then declines slightly. (Figure \@ref(fig:histogram-ages-sterilizing-operation)) The proportion of females in each group that have had sterilizing operations increases with age, with the exception of an aberration in the (44, 47] years age group.  \@ref(fig:histogram-ages-sterilizing-operation)) Thus, generally females are more likely to have had a sterilizing operation than younger persons.
...
A second, even more salient difference is in the reported prevalence of female sterilizing operations being used as a method of contraception is much higher for females than for males. (Sections \@ref(methods-used) and \@ref(methods-used-1))
...
The most obvious direction for further research beyond determining what contraceptive methods are being used is the analysis of how effective the various contraceptive methods are at preventing unintended pregnancy. Knowledge that a substantial minority of somewhere between 6 and 10 million persons use multiple methods of contraception simultaneously (Section \@ref(number-of-methods)) implies that such analysis should take into account simultaneous contraception use. It might be the case, for instance, that condoms and pills used together are more effective than either method used separately.

Another opportunity for further research, once the efficacies of the various methods of contraception or combinations thereof area established, is the analysis of covariates that explain tendency to use less effective methods of contraception instead of more effective methods.

At any given point in time, several million persons at risk for unintended pregnancy do not use contraception during sexual intercourse with someone of the opposite sex. (Section \@ref(reason-for-nonuse)) This appears to be in part due to inconsistent use and in part due to long-term nonuse. (Section \@ref(consistency-of-use-of-contraception)) One opportunity for further research is the analysis of covariates that explain increased probability to engage in sexual intercourse without contraception when at risk for unintended pregnancy.

The discussion in Section \@ref(differences-in-reported-contraceptive-use-between-females-and-males) highlights the need for analysis adjusted for, at least, age as covariate in order to drawn conclusions about differences in contraceptive use between females and males in the United States, which is yet another opportunity for further research.
...
* Write discussion section of report
    * Go over results and note what to discuss
