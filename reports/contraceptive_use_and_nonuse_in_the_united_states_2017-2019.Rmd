...
) |> prettify_estimates_of_totals_and_percentages())[
  , .(total, total_CI)
] |> kable()
```
...
estimate_totals_and_percentages(
  ...
  mutate_vars('percentage', \(x) 100 * x) |>
  mutate_vars('total', \(x) x / 10e6) |>
  mutate_dt(
    total_CI = sprintf("(%.3fM, %.3fM)", total_CI_low, total_CI_high),
    percentage_CI = sprintf("(%0.1f%%, %0.1f%%)", percentage_CI_low, percentage_CI_high)
  ) |>
  flextable(
    col_keys = c(
    'description',
  ...
  colformat_double(j = 'total', suffix = 'M', digits = 3) |>
  colformat_double(j = 'percentage', suffix = '%', digits = 1) |>
  set_header_labels(values = list(
    description = '',
    total = 'Estimate',
    total_CI = '95% C.I.',
    percentage = 'Estimate',
    percentage_CI = '95% C.I.'
  )) |>
  add_header_row(
    values = c('', 'Total', 'Percentage'),
    colwidths = c(1, 2, 2)
  ) |>
  align(i = 1, part = 'header', align = 'center') |>
  align(i = 2, part = 'header', align = 'center') |>
  align(i = 1:2, j = 2:5, part = 'body', align = 'center') |>
  padding(i = 1, padding.bottom = 0, part = 'header') |>
  bold(part = 'header') |>
  border_remove() |>
...
estimate_totals_and_percentages(
  ~used_last_3m,
  get_NSFG_survey_design_for_sex('male')
) |> prettify_estimates_of_totals_and_percentages() |> kable()
```
...
)[order(-total)] |> prettify_estimates_of_totals_and_percentages() |> kable()
```
...
estimate_totals_and_percentages(
  ...
  get_NSFG_survey_design_for_sex('fem')
)[description == TRUE] |> prettify_estimates_of_totals_and_percentages() |> kable()
```
...
estimate_NSFG_mentions(
  ...
  skip = c(8, 20:21),
  format = 'METH3MF'
)[total > 0][order(-total)] |> prettify_estimates_of_totals_and_percentages() |> kable()
```
