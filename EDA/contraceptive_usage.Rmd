...
ggplot(
  ...
  mapping = aes(
    y = current_contraceptive_count_factor
  )
) +
  geom_point(
    aes(
  ...
  geom_segment(
    aes(
      yend = current_contraceptive_count_factor,
      x = `2.5 %`,
      xend = `97.5 %`
    ),
    arrow = arrow(angle = 90, ends = 'both', length = unit(0.0625, 'inches')),
...
```{r}
female_current_method_proportions[, sum(total)]
```
...
A few exceptions exist to the pairings with condom or withdrawal. One is the aforementioned combination of male and female sterilization procedures. Others include vasectomy & IUD, condom & IUD, pill & tubal ligation, and pill & vasectomy.


## To Do

* Rename "total" in output from `svyci` to "estimate"
* Make output from `svybyci` more like `svyci`
* Estimate exclusive vs paired current use of condoms and withdrawal
* Break out paired usage of condoms and withdrawal
* Investigate users for reasons other than contraception
* Think about what do about the triple method users
