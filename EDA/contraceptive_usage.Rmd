...
totals <- svybyci(~women, ~currently, fem_svy, svytotal)

totals[, 2:5] <- totals[, 2:5] / 10^6
...
```{r, females-currently-proportions, fig.width=8, fig.height=2}
ggplot(
  data = svyci(~currently, fem_svy, svymean),
  aes(
    x = total,
    y = factor(''),
