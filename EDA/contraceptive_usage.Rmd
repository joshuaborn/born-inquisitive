...
kable(svyci(~women, fem_svy, svytotal))
```
...
kable(svybyci(~women, ~MTHUSE3_factor, fem_svy, svytotal))
...
```{r}
fem_data[(curr_meth1), .N]
fem_data[!is.na(YUSEPILL1), .N]
fem_data[curr_meth1 & is.na(YUSEPILL1), .N]
fem_data[!curr_meth1 & !is.na(YUSEPILL1), .N]

fem_data[(curr_meth17), .N]
fem_data[!is.na(YUSEIUD1), .N]
fem_data[curr_meth17 & is.na(YUSEIUD1), .N]
fem_data[!curr_meth17 & !is.na(YUSEIUD1), .N]
```
...
* Clean up EDA report
    * Start numbering report files
    * Organize code chunks into data loading, data modification, helper functions, and main body (estimates and plotting) in order to turn on caching
    * Add section selecting variables needed for analysis
