...
collect_estimate(
  rbindlist(list(
    rbindlist(lapply(
      ...
      function(x) {
        svyci(
          as.formula(sprintf('~using3m_meth%d', x)),
          subset(male_2017_2019_svy, using3m_meth10),
          svytotal
        )[
          level == TRUE,
          .(level = x, estimate, se, `2.5 %`, `97.5 %`)
    ...
    as.data.table(
      svyci(
        ...
        , .(
          level = 'Exclusively CONTRACEPTIVE PATCH',
          estimate, se, `2.5 %`, `97.5 %`
        )
      ]
    )
  ...
  type = 'total',
  domain = 'males currently using contraceptive patch',
  title = 'by other contraceptive method'
) |> standard_millions_plot()
```
...
collect_estimate(
  rbindlist(list(
    rbindlist(lapply(
      ...
      function(x) {
        svyci(
          as.formula(sprintf('~using3m_meth%d', x)),
          subset(male_2017_2019_svy, using3m_meth13),
          svytotal
        )[
          level == TRUE,
          .(level = x, estimate, se, `2.5 %`, `97.5 %`)
    ...
    as.data.table(
      svyci(
        ...
        , .(
          level = 'Exclusively SOMETHING ELSE',
          estimate, se, `2.5 %`, `97.5 %`
        )
      ]
    )
  ...
  type = 'total',
  domain = 'males currently using something else',
  title = 'by other contraceptive method'
) |> standard_millions_plot()
```
