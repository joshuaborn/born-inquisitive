...
```{r same-method, dependson='process-data'}
...
cal2002bool <- copy(cal2002[, .(CASEID, cm_value, cm_label, MONSX, NUMMULT, SIMSEQ)])
...
set_mentions_to_booleans(
  ...
  out_prefix = 'meth',
  in_prefix = 'METHHIST',
  in_series = paste0('_', 1:4),
  indices = c(3:21, 24:25)
)

cal2002bool[, `:=`(
  no_method = cal2002[,
    lapply(.SD, \(col) has_level(col, 1)),
    .SDcols = paste('METHHIST', 1:4, sep = '_')
  ] |> apply(1, any),
  sterile_respondent = cal2002[,
    lapply(.SD, \(col) has_level(col, 22)),
    .SDcols = paste('METHHIST', 1:4, sep = '_')
  ] |> apply(1, any),
  sterile_partner = cal2002[,
    lapply(.SD, \(col) has_level(col, 23)),
    .SDcols = paste('METHHIST', 1:4, sep = '_')
  ] |> apply(1, any)
)]
...
```{r explore-boolean-table, dependson='boolean-table'}
cal2002bool[NUMMULT < 3, .N] / cal2002bool[!is.na(NUMMULT), .N]
cal2002bool[, tablena(NUMMULT)]
```
...
```{r clusters-exploration, dependson='boolean-table', eval=FALSE}
...
```{r sequential, dependson='boolean-table'}
cal2002bool[,
  sequential := !(
    is.na(MONSX) |
      is.na(NUMMULT) |
...
* Find priority ranking for methods from _Contraceptive Technology_ (might have also appeared in other papers)
