---
title: "Weight Adjustment of 2015-2019 NSFG using Live Birth Targets by Age and Marital Status"
author: "Joshua Born"
output:
  html_document:
    code_folding: hide
  ...
  left_join(
    NSFG_2015_2019_preg_data |>
      ...
      summarize(
        ...
        abortion_2014_age =  case_when(
          any(OUTCOME == 'INDUCED ABORTION' & Year == 2014 & AGEPREG < 20) ~ 'Under 20 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year == 2014 & AGEPREG <= 24) ~ '20-24 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year == 2014 & AGEPREG <= 29) ~ '25-29 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year == 2014 & AGEPREG <= 34) ~ '30-34 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year == 2014 & AGEPREG <= 39) ~ '35-39 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year == 2014 & AGEPREG >= 40) ~ '40 years and over',
          .default = 'Other'
        ),
        ...
        abortion_2013_age =  case_when(
          any(OUTCOME == 'INDUCED ABORTION' & Year == 2013 & AGEPREG < 20) ~ 'Under 20 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year == 2013 & AGEPREG <= 24) ~ '20-24 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year == 2013 & AGEPREG <= 29) ~ '25-29 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year == 2013 & AGEPREG <= 34) ~ '30-34 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year == 2013 & AGEPREG <= 39) ~ '35-39 years',
          any(OUTCOME == 'INDUCED ABORTION' & Year == 2013 & AGEPREG >= 40) ~ '40 years and over',
          .default = 'Other'
        ),
      ),
    by = 'CASEID'
...
NSFG_2015_2019_for_calibration |>
  count(poststrat_age_race) |>
  gt()

NSFG_2015_2019_for_calibration |>
  count(birth_2014_age) |>
  gt()

NSFG_2015_2019_for_calibration |>
  count(birth_2013_age) |>
  gt()

NSFG_2015_2019_for_calibration |>
  count(birth_2012_age) |>
  gt()

NSFG_2015_2019_for_calibration |>
  count(birth_2011_age) |>
  gt()

NSFG_2015_2019_for_calibration |>
  count(birth_2014_marital_status) |>
  gt()

NSFG_2015_2019_for_calibration |>
  count(birth_2013_marital_status) |>
  gt()

NSFG_2015_2019_for_calibration |>
  count(birth_2012_marital_status) |>
  gt()

NSFG_2015_2019_for_calibration |>
  count(birth_2011_marital_status) |>
  gt()

NSFG_2015_2019_for_calibration |>
  mutate(abortion_in_2014 = (abortions_in_2014 > 0)) |>
  count(abortion_in_2014) |>
  gt()

NSFG_2015_2019_for_calibration |>
  mutate(abortion_in_2013 = (abortions_in_2013 > 0)) |>
  count(abortion_in_2013) |>
  gt()

NSFG_2015_2019_for_calibration |>
  count(abortion_2014_age) |>
  gt()

NSFG_2015_2019_for_calibration |>
  count(abortion_2013_age) |>
  gt()
```


### Create NSFG Survey Objects

```{r create-NSFG-survey-objects, dependson="select-NSFG-variables"}
...
NSFG_2015_2019_preg_svy <- NSFG_2015_2019_preg_data |>
  select(-WGT2015_2019) |>
  left_join(
...
### Load Target Totals for Calibration

```{r load-target-totals-for-calibration}
poststrat_pop_targets <- readRDS(
...
poststrat_pop_total <- sum(poststrat_pop_targets$Freq)
...
poststrat_targets <- c(list(poststrat_pop_targets), NVSS_age_2011_2014, NVSS_marital_status_2011_2014)
#poststrat_targets <- list(poststrat_pop_targets)

Guttmacher_abortion_totals <- readRDS(here('data/guttmacher_APC_national.Rds'))
```


### Create Calibrated NSFG Survey Object

```{r create-calibrated-NSFG-survey-objects, dependson=c("create-NSFG-survey-objects", "load-target-totals-for-calibration")}
source(here('R/rake_with_factor_adjustment.R'))

poststrat_abortions_total <- Guttmacher_abortion_totals |>
  filter(year %in% 2013:2014) |>
  pull(abortionstotal) |>
  sum()

NSFG_2015_2019_fem_cal_svy <- rake_with_factor_adjustment(
  NSFG_2015_2019_fem_svy,
  ...
  factor.estimation.target = poststrat_abortions_total,
  factor.adjusted.vars = ~any_abortion,
  equalize = TRUE,
  control = list(maxit=100, epsilon=1, verbose=FALSE)
)

NSFG_2015_2019_preg_cal_svy <- NSFG_2015_2019_preg_data |>
  select(-WGT2015_2019) |>
  left_join(
    NSFG_2015_2019_fem_cal_svy |>
      as_data_frame_with_weights() |>
      select(CASEID, contains('_WGT')),
    by = join_by('CASEID')
  ) |>
...
### Verify Post-Stratification

#### Births

```{r verify-post-stratification-births, dependson=c("create-NSFG-survey-objects", "create-calibrated-NSFG-survey-objects")}
verify_poststratification <- function(svy_before, svy_after, targets, variable_name) {
  svy_before |>
    group_by(get(variable_name)) |>
    summarize(
      Uncal = survey_total(one, vartype = 'ci')
    ) |>
    rename_with(~ gsub('get(variable_name)', variable_name, .x, fixed = TRUE)) |>
    full_join(
      svy_after |>
        group_by(get(variable_name)) |>
        summarize(
          Calib = survey_total(one, vartype = 'ci')
        ) |>
        rename_with(~ gsub('get(variable_name)', variable_name, .x, fixed = TRUE))
    ) |>
    full_join(
    ...
    mutate(
      Uncal_Bias = Uncal - Freq,
      Uncal_Rel_Bias = Uncal_Bias / Freq,
      `Uncal_Covers?` = Freq <= Uncal_upp & Freq >= Uncal_low,
      Calib_Bias = Calib - Freq,
      Calib_Rel_Bias = Calib_Bias / Freq,
      Diff = Calib - Uncal
    ) |>
    select(
      matches(variable_name), Diff, Uncal, Calib, Freq, Calib_low, Calib_upp, Calib_Rel_Bias
    )
}

verify_poststratification(
    NSFG_2015_2019_fem_svy,
    NSFG_2015_2019_fem_cal_svy,
    poststrat_pop_targets,
    'poststrat_age_race'
  ) |>
  gt() |>
  fmt_number(where(is.numeric), n_sigfig = 4, suffix = 'k') |>
  fmt_percent(Calib_Rel_Bias, decimals = 1)
...
```{r verify-post-stratification-abortions, dependson=c("create-NSFG-survey-objects", "create-calibrated-NSFG-survey-objects")}
NSFG_2015_2019_fem_svy |>
  summarize(
    n = unweighted(sum(abortions_in_2013)),
    Uncal = survey_total(abortions_in_2013, vartype = 'ci'),
  ) |>
  mutate(Year = 2013, .before = everything()) |>
  bind_rows(
    NSFG_2015_2019_fem_svy |>
      summarize(
        n = unweighted(sum(abortions_in_2014)),
        Uncal = survey_total(abortions_in_2014, vartype = 'ci')
      ) |>
  ...
  full_join(
    NSFG_2015_2019_fem_cal_svy |>
      summarize(
        Calib = survey_total(abortions_in_2013, vartype = 'ci'),
      ) |>
      mutate(Year = 2013, .before = everything()) |>
      bind_rows(
        NSFG_2015_2019_fem_cal_svy |>
          summarize(
            Calib = survey_total(abortions_in_2014, vartype = 'ci')
          ) |>
          mutate(Year = 2014, .before = everything())
      ),
    by = join_by(Year)
  ...
  full_join(
    Guttmacher_abortion_totals |>
      rename(Year = year, Freq = abortionstotal) |>
      filter(Year %in% 2013:2014) |>
      select(Year, Freq),
    by = join_by(Year)
  ) |>
  mutate(
    Diff = Calib - Uncal,
    Calib_Rel_Bias = (Freq - Calib) / Freq
  ) |>
  select(Year, n, Uncal, Calib, Diff, Freq, Calib_low, Calib_upp, Calib_Rel_Bias) |>
  gt() |>
  fmt_number(Uncal:Calib_upp, n_sigfig = 4, suffix = 'k') |>
  fmt_percent(Calib_Rel_Bias, decimals = 1)
```
...
```{r summarize-weights, dependson=c("create-NSFG-survey-objects", "create-calibrated-NSFG-survey-objects")}
summarize_weights <- function(x, label, useUWE = TRUE) {
  tbl <- tibble(
    n = sum(!is.na(x)),
    missing = sum(is.na(x)),
    positive = sum(x > 0),
...
summarize_weights(
  weights(NSFG_2015_2019_fem_svy, type = 'sampling'),
  'original_weights'
) |> full_join(
  summarize_weights(
    weights(NSFG_2015_2019_fem_cal_svy, type = 'sampling'),
    'calibrated_weights'
  ),
  ...
  fmt_number(rows = 1:5, decimals = 0) |>
  fmt_number(rows = 17, decimals = 3)
```
...
```{r estimate-births-uncalibrated, dependson="create-NSFG-survey-objects"}
NSFG_2015_2019_unc_est_births <- bind_rows(
  NSFG_2015_2019_preg_svy |>
    filter(
      Year %in% 2011:2014 & OUTCOME == 'LIVE BIRTH'
    ) |>
    group_by(Year, OUTCOME, age_group) |>
    summarize(
      Unc_Est = survey_total(NBRNLV_S, vartype = 'ci'),
...
```{r estimate-abortions-uncalibrated, dependson="create-NSFG-survey-objects"}
estimate_abortions_by_domain <- function(variable, label) {
  ...
  NSFG_2015_2019_preg_svy |>
    ...
    summarize(
      n = unweighted(sum(one)),
      Unc_Est = survey_total(one, vartype = 'ci'),
      .groups = 'drop'
    ) |>
  ...
  NSFG_2015_2019_preg_svy |>
    ...
    mutate(
      ...
      Level = sub('^.*_age_', '', name) %>% 
        ...
        str_to_sentence(),
      ColName = case_when(
        grepl('_low', name) ~ 'Unc_Est_low',
        grepl('_upp', name) ~ 'Unc_Est_upp',
        grepl('Est_', name) ~ 'Unc_Est',
        grepl('n_', name) ~ 'n'
      )
    ) |>
    pivot_wider(
...
## Estimation, Calibrated

### Estimate Births, Calibrated

```{r estimate-births-calibrated, dependson="create-calibrated-NSFG-survey-objects"}
NSFG_2015_2019_cal_est_births <- bind_rows(
  NSFG_2015_2019_preg_cal_svy |>
    filter(
      Year %in% 2011:2014 & OUTCOME == 'LIVE BIRTH'
    ) |>
    group_by(Year, OUTCOME, age_group) |>
    summarize(
      Cal_Est = survey_total(NBRNLV_S, vartype = 'ci'),
      n = unweighted(sum(one))
    ) |>
    ungroup() |>
    select(-OUTCOME) |>
...
### Estimate Abortions, Calibrated

```{r estimate-abortions-calibrated, dependson="create-NSFG-survey-objects"}
estimate_abortions_by_domain <- function(variable, label) {
  external_vector <- c(Level = variable)
   NSFG_2015_2019_preg_cal_svy |>
    filter(
    ...
    summarize(
      n = unweighted(sum(one)),
      Cal_Est = survey_total(one, vartype = 'ci'),
      .groups = 'drop'
    ) |>
    rename(any_of(external_vector)) |>
    mutate(
...
NSFG_2015_2019_cal_est_abortions <- bind_rows(
   NSFG_2015_2019_preg_cal_svy |>
    filter(
      Year %in% 2011:2014 & OUTCOME == 'INDUCED ABORTION'
    ) |>
    ...
    mutate(
      ...
      Level = sub('^.*_age_', '', name) %>% 
        ...
        str_to_sentence(),
      ColName = case_when(
        grepl('_low', name) ~ 'Cal_Est_low',
        grepl('_upp', name) ~ 'Cal_Est_upp',
        grepl('Est_', name) ~ 'Cal_Est',
        grepl('n_', name) ~ 'n'
      )
    ) |>
    pivot_wider(
...
full_join(
  ...
  full_join(
    NSFG_2015_2019_cal_est_births,
    by = c('Year', 'Variable', 'Level'),
    suffix = c('.unc', '.cal')
  ) |>
  mutate(
    unequal_n = n.unc != n.cal,
    Unc_Bias = Unc_Est - Births,
    Unc_Rel_Bias = Unc_Bias / Births,
    `Unc_Covers?` = Births <= Unc_Est_upp & Births >= Unc_Est_low,
    Cal_Bias = Cal_Est - Births,
    Cal_Rel_Bias = Cal_Bias / Births,
    `Cal_Covers?` = Births <= Cal_Est_upp & Births >= Cal_Est_low,
    Bias_Diff = abs(Cal_Bias) - abs(Unc_Bias),
    Est_Diff = Cal_Est - Unc_Est,
    Rel_Bias_Diff = abs(Cal_Rel_Bias) - abs(Unc_Rel_Bias),
    `Fixed?` = if_else(
        !`Unc_Covers?`,
        `Cal_Covers?`,
        NA
    ),
    `Broke?` = `Unc_Covers?` & !`Cal_Covers?`
  ) |>
  rename(
    n = n.cal
  ) |>
...
test_harness_abortions |>
  ...
  left_join(
    NSFG_2015_2019_cal_est_abortions,
    by = c('Year', 'Variable', 'Level'),
    suffix = c('.unc', '.cal')
  ) |>
  mutate(
    Unc_Bias = Unc_Est - Abortions,
    Cal_Bias = Cal_Est - Abortions,
    Unc_Rel_Bias = Unc_Bias / Abortions,
    Cal_Rel_Bias = Cal_Bias / Abortions,
    `Unc_Covers?` = Abortions <= Unc_Est_upp & Abortions >= Unc_Est_low,
    `Cal_Covers?` = Abortions <= Cal_Est_upp & Abortions >= Cal_Est_low,
    Bias_Diff = abs(Cal_Bias) - abs(Unc_Bias),
    Est_Diff = Cal_Est - Unc_Est,
    Rel_Bias_Diff = abs(Cal_Rel_Bias) - abs(Unc_Rel_Bias),
    `Fixed?` = if_else(
...
### Compare Abortions Estimates by Age

There is a pattern in which abortions among younger people are overestimated and abortions among older people are underestimated, though where the cut point between over- and under-estimation occurs varies by year. 

This can be handled by extending the weight adjustment to abortion targets per year per age group. The `rake_with_factor_adjustment` function would need to be extended to handle multiple target margins for the `factor.` arguments, much as it does already for `postStratify` calls.

However, I am not sure this is a wise use of work. There are 72 respondents who report abortions in 2013, and 68 respondents who report abortions in 2014, for a total of 140 respondents. This is not an unreasonable number of respondents for to base weight adjustment upon. However, if we intersect abortions-per-year with age of woman, then the number of respondents in each cell will decrease considerably. 

In particular, there would be 14 respondents in the under 20 years age group, 52 respondents in the 20-24 years age group, 40 respondents in the 25-29 years age group, 16 respondents in the 30-34 years age group, 13 respondents in the 35-39 years age group, and 5 respondents in the 40 years and over age group.

This might be an insufficient number of respondents in each cell on which to base weight adjustment. Perhaps some cells could be collapsed, particularly all the 30 years or older age groups could be collapsed into one cell. This would at least lead to cell counts of 14, 52, 40, and 34. 


```{r compare-abortions-estimates-by-age, dependson=c("create-abortions-test-harness", "estimate-births-uncalibrated")}
test_harness_abortions |>
  mutate(
    Level = case_match(
      Level,
  ...
  filter(n > 5 & Variable == 'Age group') |>
  pivot_wider(
    names_from = 'Year',
    values_from = c('n', 'Abortions', 'Cal_Est', 'Cal_Rel_Bias')
  ) |>
  gt() |>
