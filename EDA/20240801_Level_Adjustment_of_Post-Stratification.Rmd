...
```{r with-1-levels, dependson=c("select-pregnancies-variables", "select-female-respondents-variables")}
derived_fem <- data_fem |>
  mutate(
    poststrat_age_race = factor(gsub(
      '[ -]',
  ...
  left_join(
    data_preg |>
      group_by(CASEID) |>
      summarize(
        abortion_2011_2014 = if_else(
          any(OUTCOME == 'INDUCED ABORTION' & Year %in% 2011:2014),
          'Abortion reported',
          'No abortion reported'
        ),
        abortion_2011plus = if_else(
  ...
  mutate(
    across(
      ...
      function(x) {
        factor(
          coalesce(x, 'No abortion reported'),
          levels = c(
            'Abortion reported',
            'No abortion reported'
          )
...
guttmacher_APC_2011_2014 <- guttmacher_APC_national |>
  filter(year %in% 2011:2014) |>
  select(abortionstotal) |>
  summarize(
    Freq = sum(abortionstotal)
  ) |>
  mutate(
    abortion_2011_2014 = 'Abortion reported',
    .before = everything()
  ) %>%
  add_row(
    abortion_2011_2014 = 'No abortion reported',
    Freq = census_pop_total_2015_2019 - pull(., Freq)
  ) |>
  mutate(
    abortion_2011_2014 = factor(
      abortion_2011_2014,
      levels = c(
        'Abortion reported',
        'No abortion reported'
