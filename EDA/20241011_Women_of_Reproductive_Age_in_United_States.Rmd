---
title: "Number of Women of Reproductive Age in the United States"
author: "Joshua Born"
output:
  html_document:
    code_folding: hide
    toc: true
    toc_depth: 5
    css: wide.css
---


```{r setup, include=FALSE}
library(dplyr)
library(ggplot2)
library(gt)
library(here)
library(readr)
library(readxl)

knitr::opts_chunk$set(
  cache = TRUE,
  dpi=200,
  echo = TRUE,
  fig.align = 'center',
  fig.height=6.4,
  fig.retina=1.5,
  fig.width=9.6
)
```


## Loading Data

### For 1940-1979

https://www.census.gov/data/tables/time-series/demo/popest/pre-1980-national.html

```{r load-data-from-excel-files, message=FALSE, warning=FALSE}
load_Census_year <- function(year) {
  read_xls(
    here(paste0('data/Census/pe-11-', 10 * (year %/% 10), 's.xls')),
    sheet = as.character(year),
    skip = 9,
    col_names = FALSE
  ) |>
  as_tibble() %>%
  mutate(
    Year = year,
    Age = as.integer(.[[1]]),
    Population.Female.All = as.integer(.[[4]])
  ) |>
  filter(!is.na(Age)) |>
  select(Year, Age, Population.Female.All)
}

Census_1940_1979 <- Reduce(
  \(x, y) bind_rows(list(x, load_Census_year(y))),
  1941:1979,
  load_Census_year(1940)
)
```


### For 1980-1989

https://www.census.gov/data/datasets/time-series/demo/popest/1980s-national.html

```{r load-data-from-fixed-width-file}
load_Census_fixed_width <- function(code) {
  read_fwf(
    here(paste0('data/Census/1980-1989/E', code, 'RQI.TXT')),
    fwf_cols(
      Series = c(1, 2),
      Month = c(3, 4),
      Year = c(5, 6),
      Age = c(7, 10),
      Population = c(12, 21),
      Population.Male.All = c(22, 31),
      Population.Female.All = c(32, 41)
    ),
    show_col_types = FALSE
  )
}

Census_1980_1989 <- Reduce(
  \(x, y) bind_rows(list(x, load_Census_fixed_width(y))),
  c(8182, 8283, 8384, 8485, 8586, 8687, 8788, 8889, 8990),
  load_Census_fixed_width(8081)
)
```


```{r data-processing, dependson=c("load-data-from-excel-files", "load-data-from-fixed-width-file")}
Census_by_age <- bind_rows(list(
  Census_1940_1979,
  Census_1980_1989 |>
    filter(Month == 7) |>
    mutate(Year = Year + 1900)
)) |>
  mutate(
    across(c(Year, Population.Female.All), as.integer),
    Age.Group = cut(
      Age,
      c(14, 19, 24, 29, 34, 39, 44),
      labels = c(
        "[15,19]",
        "[20,24]",
        "[25,29]",
        "[30,34]",
        "[35,39]",
        "[40,44]"
      ),
      ordered_result = TRUE
    )
  ) |>
  filter(!is.na(Age.Group)) |>
  group_by(Year, Age.Group) |>
  summarize(Population.Female = sum(Population.Female.All), .groups = "drop_last")

Census_reproductive_age <- Census_by_age |>
  summarize(Population.Female.Reproductive.Age = sum(Population.Female), .groups = "drop")
```


```{r women-of-reproductive-age-by-age-group-plot, dependson="data-processing", warning=FALSE}
Census_by_age |>
  ggplot(
    mapping = aes(
      x = Year,
      y = Population.Female,
      color = Age.Group,
      shape = Age.Group
    )
  ) +
    geom_line(
      alpha = 0.4,
      linewidth = 0.75
    ) +
    geom_point(
      fill = 'white',
      size = 1.5
    ) +
    scale_y_continuous(,
      breaks = \(x) seq(0, x[2]+1e6, 5e5),
      expand = expansion(mult = c(0, 0.05)),
      labels = scales::label_number(suffix = "M", scale = 1e-6),
      limits = c(4e6, NA),
      minor_breaks = NULL
    ) +
    scale_x_continuous(
      breaks = \(x) seq(x[1]+1, x[2]-1, 2),
      expand = expansion(add = 1),
      minor_breaks = \(x) seq(x[1]+1, x[2]-1, 1)
    ) +
    scale_color_brewer(
      palette = 2,
      type = "qual"
    ) + 
    labs(
      x = 'Year',
      y = 'Female Population of United States by Age Group'
    ) +
    theme_classic() +
    theme(
      axis.line.x.bottom = element_line(
        color = 'black',
        linewidth = 0.3
      ),
      axis.line.y.left = element_line(
        color = 'black',
        linewidth = 0.3
      ),
      axis.text.x = element_text(
        angle = 45,
        color = 'black',
        hjust = 0.5,
        vjust = 0.65
      ),
      axis.text.y = element_text(
        color = 'black',
        margin = margin(r = 0)
      ),
      axis.ticks.y = element_blank(),
      axis.ticks.x = element_line(
        color = 'black',
        linewidth = 0.2
      ),
      axis.title.x = element_text(
        margin = margin(t = 2)
      ),
      axis.title.y = element_text(
        margin = margin(r = 10)
      ),
      panel.grid.major = element_line(
        color = 'gray',
        linetype = 3,
        linewidth = 0.25
      ),
      panel.grid.minor = element_line(
        color = 'gray',
        linetype = 3,
        linewidth = 0.25
      ),
      panel.grid.minor.x = element_blank(),
      plot.margin = margin(t = 0)
  )
```


## Plots

```{r total-women-of-reproductive-age-plot, dependson="data-processing"}
Census_reproductive_age |>
  ggplot(
    mapping = aes(
      x = Year,
      y = Population.Female.Reproductive.Age
    )
  ) +
    geom_line(
      alpha = 0.4,
      color = 'blue',
      linewidth = 1.5
    ) +
    geom_point(
      color = 'blue',
      fill = 'white',
      size = 2,
      shape = 21
    ) +
    scale_shape_manual(
      values = 21
    ) +
    scale_y_continuous(,
      breaks = \(x) seq(0, x[2]+1e6, 2e6),
      labels = scales::label_number(suffix = "M", scale = 1e-6),
    ) +
    scale_x_continuous(
      breaks = \(x) seq(x[1]+1, x[2]-1, 2),
      expand = expansion(add = 1),
      minor_breaks = \(x) seq(x[1]+1, x[2]-1, 1)
    ) +
    labs(
      x = 'Year',
      y = 'Female Population of United States of Reproductive Age (15-44 years)'
    ) +
    theme_classic() +
    theme(
      axis.line.x.bottom = element_line(
        color = 'black',
        linewidth = 0.3
      ),
      axis.line.y.left = element_line(
        color = 'black',
        linewidth = 0.3
      ),
      axis.text.x = element_text(
        angle = 45,
        color = 'black',
        hjust = 0.5,
        vjust = 0.65
      ),
      axis.text.y = element_text(
        color = 'black',
        margin = margin(r = 0)
      ),
      axis.ticks.y = element_blank(),
      axis.ticks.x = element_line(
        color = 'black',
        linewidth = 0.2
      ),
      axis.title.x = element_text(
        margin = margin(t = 2)
      ),
      axis.title.y = element_text(
        margin = margin(r = 10)
      ),
      panel.grid.major = element_line(
        color = 'gray',
        linetype = 3,
        linewidth = 0.25
      ),
      panel.grid.minor = element_line(
        color = 'gray',
        linetype = 3,
        linewidth = 0.25
      ),
      panel.grid.minor.x = element_blank(),
      plot.margin = margin(t = 0)
  )
```

