...
guttmacher_2014 <- guttmacher_national |>
  filter(year == 2014) |>
  select(starts_with('abortions'))

abortionstotal2014 <- pull(guttmacher_2014, abortionstotal)

guttmacher_2012 <- guttmacher_national |>
  filter(year == 2012) |>
  select(starts_with('abortions'))

abortionstotal2012 <- pull(guttmacher_2012, abortionstotal)

guttmacher_2011 <- guttmacher_national |>
  filter(year == 2011) |>
  select(starts_with('abortions'))

abortionstotal2011 <- pull(guttmacher_2011, abortionstotal)
```
...
guttmacher2014 <- bind_rows(
  ...
  PercentageAPS2014 = PercentageAPS2014 / 100,
  TotalAPS2014 = as.integer(round(abortionstotal2014 * PercentageAPS2014)),
  .before = 'TotalAPC2014'
)
...
CDC2014 <- bind_rows(
  ...
  Level = str_squish(Level),
  PercentageCDC = PercentageCDC / 100,
  TotalCDC = as.integer(round(abortionstotal2014 * PercentageCDC)),
  .before = 'TotalAPC'
)
...
CDC2012 <- bind_rows(
  ...
  Level = str_squish(Level),
  PercentageCDC = PercentageCDC / 100,
  TotalCDC = as.integer(round(abortionstotal2012 * PercentageCDC)),
  .before = 'TotalAPC'
)
...
CDC2011 <- bind_rows(
  ...
  Level = str_squish(Level),
  PercentageCDC = PercentageCDC / 100,
  TotalCDC = as.integer(round(abortionstotal2011 * PercentageCDC)),
  .before = 'TotalAPC'
)
...
CDC_combined <- full_join(
  ...
  full_join(
    rename_with(
      CDC2011,
      ~paste0(.x, '2011'),
      starts_with(c('Percentage', 'Total'))
    ),
    by = c('Variable', 'Level')
  ) |>
  select(-contains('APC'))
...
full_join(
  select(guttmacher2014, -TotalAPC2014),
  CDC_combined,
  by = c('Variable', 'Level')
) |>
  arrange(Variable) |>
