...
colnames(indicators) <- gsub('_group|HISPRACE2', '', colnames(indicators))
...
NSFG_2015_2019_for_calibration <- bind_cols(
  ...
  left_join(
    NSFG_2015_2019_preg_data |>
      mutate(
        calibrate_abortion2014 = if_else(
          OUTCOME == 'INDUCED ABORTION' & Year == 2014,
          1,
          0
        ),
        calibrate_abortion2013 = if_else(
          OUTCOME == 'INDUCED ABORTION' & Year == 2013,
          1,
          0
        ),
        calibrate_abortion2012 = if_else(
          OUTCOME == 'INDUCED ABORTION' & Year == 2012,
          1,
          0
        ),
        calibrate_abortion2011 = if_else(
          OUTCOME == 'INDUCED ABORTION' & Year == 2011,
          1,
          0
        ),
        calibrate_birth2014 = if_else(
          OUTCOME == 'LIVE BIRTH' & Year == 2014,
          1,
          0
        ),
        calibrate_birth2013 = if_else(
          OUTCOME == 'LIVE BIRTH' & Year == 2013,
          1,
          0
        ),
        calibrate_birth2012 = if_else(
          OUTCOME == 'LIVE BIRTH' & Year == 2012,
          1,
          0
        ),
        calibrate_birth2011 = if_else(
          OUTCOME == 'LIVE BIRTH' & Year == 2011,
          1,
          0
        )
      ) |>
      select(CASEID, starts_with('calibrate_')) |>
      group_by(CASEID) |>
      summarize(across(everything(), sum)),
    by = 'CASEID'
  ) |> 
  mutate(across(starts_with('calibrate'), \(x) coalesce(x, 0)))
...
```{r count-cells-for-calibration, dependson="set-calibration-flags"}
NSFG_2015_2019_for_calibration |>
  select(c(contains('Hispanic'), starts_with('calibrate_'))) |>
  map(table) |>
  bind_rows(.id = 'Cell Description') |>
  gt()
```
