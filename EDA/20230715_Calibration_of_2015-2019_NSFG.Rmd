...
NVSS_combined <- bind_rows(
  read_tsv(
    ...
    filter(is.na(Notes)) |>
    rename(Level = Age.of.Mother.9) |>
    select(Year, Level, Births) |>
    mutate(Variable = 'Age group', .after = Year),
  read_tsv(
      here('data/NVSS/2011-2014/education.txt'),
      name_repair = 'universal',
      show_col_types = FALSE
    ) |>
    filter(is.na(Notes)) |>
    rename(Level = Mother.s.Education) |>
    select(Year, Level, Births) |>
...
NSFG_svy_preg_2015_2019 <- NSFG_data_preg2015_2019 |>
  as_survey_design(
    strata = SEST,
    ids = SECU,
...
```{r estimate-uncalibrated, dependson="create-NSFG-survey-object"}
est_unc_totals <- NSFG_svy_preg_2015_2019 |>
  group_by(Year, OUTCOME) |>
  summarize(
    Est_Preg = survey_total(one, vartype = 'ci'),
...
est_unc_by_age_group <- NSFG_svy_preg_2015_2019 |>
  group_by(Year, OUTCOME, age_group) |>
  summarize(
    Est_Births = survey_total(one, vartype = 'ci'),
    n = unweighted(sum(one))
  ) |>
  filter(
    Year %in% 2011:2014 &
      OUTCOME %in% c('LIVE BIRTH', 'INDUCED ABORTION')
  )
```


## Comparison of Uncalibrated NSFG Estimates with NVSS Live Births

```{r compare-uncalibrated-estimates-with-NVSS, dependson=c("create-NVSS-test-harness", "estimate-uncalibrated")}
full_join(
  NVSS_combined,
  est_unc_by_age_group |>
    ungroup() |>
