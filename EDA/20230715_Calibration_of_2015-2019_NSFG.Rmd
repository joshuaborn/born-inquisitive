...
NSFG_2015_2019_est_unc_abortions <- bind_rows(
  ...
  NSFG_2015_2019_preg_srv |>
    group_by(Year, OUTCOME, gestational_age) |>
    summarize(
      Est_Abortions = survey_total(one, vartype = 'ci'),
      n = unweighted(sum(one))
    ) |>
    ungroup() |>
    rename(Level = gestational_age) |>
    mutate(
      Variable = 'Gestational age'
    ) |>
    filter(
  ...
  NSFG_2015_2019_preg_srv |>
    ...
    select(-OUTCOME),
  NSFG_2015_2019_preg_srv |>
    group_by(Year, OUTCOME, relationship_status) |>
    summarize(
      Est_Abortions = survey_total(one, vartype = 'ci'),
      n = unweighted(sum(one))
    ) |>
    ungroup() |>
    rename(Level = relationship_status) |>
    mutate(
      Variable = 'Relationship status'
    ) |>
    filter(
    ...
    select(-OUTCOME),
  NSFG_2015_2019_preg_srv |>
    group_by(Year, OUTCOME, previous_births_topcoded4) |>
    summarize(
      Est_Abortions = survey_total(one, vartype = 'ci'),
      n = unweighted(sum(one))
    ) |>
    ungroup() |>
    rename(Level = previous_births_topcoded4) |>
    mutate(
      Variable = 'No. of prior births'
    ) |>
    filter(
    ...
    select(-OUTCOME),
  NSFG_2015_2019_preg_srv |>
    group_by(Year, OUTCOME, previous_births_topcoded2) |>
    summarize(
      Est_Abortions = survey_total(one, vartype = 'ci'),
      n = unweighted(sum(one))
    ) |>
    ungroup() |>
    rename(Level = previous_births_topcoded2) |>
    mutate(
      Variable = 'No. of prior births'
    ) |>
    filter(
    ...
    select(-OUTCOME),
  NSFG_2015_2019_preg_srv |>
    group_by(Year, OUTCOME, poverty_level) |>
    summarize(
      Est_Abortions = survey_total(one, vartype = 'ci'),
      n = unweighted(sum(one))
    ) |>
    ungroup() |>
    rename(Level = poverty_level) |>
    mutate(
      Variable = 'Family income as % of poverty level'
    ) |>
    filter(
