...
```{r, helper-functions}
in_millions <- function(dt, do_round = TRUE, round_to = 1) {
  if (do_round) {
    cbind(
      Filter(\(col) !is.numeric(col), dt),
      round(Filter(is.numeric, dt) / 10^6, round_to)
    )
  } else {
    cbind(
      Filter(\(col) !is.numeric(col), dt),
      Filter(is.numeric, dt) / 10^6
    )
  }
}
```
...
```{r}
kable(svyci(~count, fem_svy, svytotal))
```
...
kable(svybyci(~count, ~MTHUSE3_factor, fem_svy, svytotal))

kable(svyci(~MTHUSE3_factor, fem_svy, svymean))
```

```{r, females-currently-totals, fig.height=8, fig.width=6}
female_currently_totals <- in_millions(svybyci(~count, ~currently, fem_svy, svytotal))
...
female_currently_totals2 <- in_millions(svybyci(~count, ~currently2, fem_svy, svytotal))
...
female_number_current_methods_totals <- in_millions(svybyci(~count, ~current_contraceptive_count_factor, female_current_contraceptors, svytotal))
...
female_current_method_totals <- in_millions(rbind(
...
female_current_exclusive_method_totals <- in_millions(svybyci(~count, ~METH3M1_factor, female_current_exclusive, svytotal))
...
female_current_pair_totals <- in_millions(svybyci(~count, ~meth3m_pair_factor, female_current_pair, svytotal))
...
female_current_condom_exclusive_totals <- svybyci(~count, ~current_condom_exclusive, female_current_condom_users, svytotal)
...
female_current_withdrawal_exclusive_totals <- svybyci(~count, ~current_withdrawal_exclusive, female_current_withdrawal_users, svytotal)
...
total_using_another_method <- function(meth) {
  ...
  svybyci(~count, ~I(current_contraceptive_count > 1), subdomain, svytotal)
}

totals_using_another_method <- in_millions(rbindlist(lapply(
...
```{r}
female_calendar_users <- subset(fem_svy, curr_meth8)

female_current_calendar_totals <- rbindlist(lapply(
  c(1:7, 9:13, 15, 17:19, 23:24),
  function(i) {
    svyci(
      as.formula(sprintf('~curr_meth%d', i)),
      female_calendar_users,
      svytotal
    )[
      level == TRUE
    ][
      , .(index = i, estimate, se, `2.5 %`, `97.5 %`)
    ]
  }
))

female_current_calendar_proportions <- rbindlist(lapply(
  c(1:7, 9:13, 15, 17:19, 23:24),
  function(i) {
    svyci(
      as.formula(sprintf('~curr_meth%d', i)),
...
female_current_calendar_estimates <- female_current_calendar_totals[
  female_current_calendar_proportions,
  on = 'index'
]

female_current_calendar_estimates[
  , level := factorize(index, 'METH3MF', fem_formats)
]

female_exclusive_calendar_estimate <- cbind(
  svyci(
    ...
    , .(index = 0, estimate, se, `2.5 %`, `97.5 %`, level = 'Exclusively calendar rhythm, Standard Days, or Cycle Beads method')
  ],
  svyci(
    ~I(current_contraceptive_count == 1),
    female_calendar_users,
...
female_current_calendar_estimates <- in_millions(rbind(
  female_current_calendar_estimates,
  female_exclusive_calendar_estimate
))

setcolorder(female_current_calendar_estimates, c('index', 'level'))

kable(female_current_calendar_estimates[order(-estimate)])
```

```{r, female-current-calendar-totals, fig.width=12, fig.height=5}
ggplot(
  data = female_current_calendar_estimates[estimate > 0],
  mapping = aes(
...
```{r, female-current-calendar-proportions, fig.width=12, fig.height=5}
ggplot(
  data = female_current_calendar_estimates[estimate > 0],
  aes(
    y = reorder(level, prop.estimate)
  )
) +
  geom_point(
    aes(
  ...
  geom_segment(
    aes(
      yend = reorder(level, prop.estimate),
      x = `prop.2.5%`,
      xend = `prop.97.5%`
    ),
    arrow = arrow(angle = 90, ends = 'both', length = unit(0.0625, 'inches')),
...
* Clean up EDA report
    * Consolidate vim hack code using `lapply` and `rbindlist`
    * Combine all estimates for specific analysis, e.g., "current contraceptive usage among females" into a single data.table
        * Removes need to have naming scheme to manage numerous variables
        * Makes it easy down the road to compare 2015-2017 with 2017-2019 estimates in order to evaluate whether to combine data sets for analysis
