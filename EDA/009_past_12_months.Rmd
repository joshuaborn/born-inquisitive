...
collect_estimate(
  ...
  type = 'total',
  domain = 'females who have had sexual intercourse with a male in the past 12 months',
...
collect_estimate(
  svyci(
    ...
    subset(
      fem_2017_2019_svy,
      SEX12MO & anyuse12m_count > 0
    ),
    svytotal
  ),
  type = 'total',
  domain = 'females who used at least one method of contraception in the past 12 months',
  title = 'by how frequently any contraceptive was used in past 12 months'
) |> kable()
```
...
collect_estimate(
  ...
  type = 'total',
  domain = 'males who used at least one method of contraception in the past 12 months with their wife or cohabitating partner',
  title = 'by how frequently any contraceptive was used in past 12 months'
) |> kable()
```
...
collect_estimate(
  ...
  type = 'total',
  domain = 'males who used at least one method of contraception in the past 12 months their last partner',
  title = 'by how frequently any contraceptive was used in past 12 months'
) |> kable()
```
...
```{r, dependson=c('male-process-data')}
male_2017_2019_data$PXANYUSE2 |> table() |> kable()
male_2017_2019_data$PXANYUSE3 |> table() |> kable()
```

There aren't enough respondents in who are "Yes" to either `PXANYUSE2` and `PXANYUSE3` to estimate these domains.

The estimates of females who used at least one method of contraception in the past 12 months, but not at last intercourse were already based on too small of a sample (within the subdomain) to be useful. This will only be worse with the males segmented by partner, so I'm just going to skip those.
