...
fem_2017_2019_data[
  mos12_consistency == 'Not Ascertained',
  .(
...
The respondents in the above table should have been asked `PXNOFREQ`. Indeed, most of them should have been asked `P12MOCON`. This is inexplicable, since they are in the universe for either `P12MOCON` or `PXNOFREQ`. They will be marked "Not Ascertained" and included in the estimates, along with the "Don't know" and "Refused."
...
Narrowing the domain to just those females who were sexually active in the past 3 months reduces the "Not Ascertained" values.
...
Making the universe match `PST4WKSX` seems to eliminate the "Not Ascertained" values.
...
Ultimately, there are 160 respondents who should have been asked `MISSPILL`, but were not. This is a greater proportion of the universe than seen previously. It is worth looping back at the end and following up with the NSFG folks about these "Not Ascertained" situations when they start to affect the actual estimates.
...
Not all condom users in the past 12 months have either `P12MOCON` or `P12MOCONO` responses.

```{r, dependson='process-male-data'}
male_2017_2019$Data[
  (SEXSTAT %in% c(1, 2, 6) | is.na(SEXSTAT)) &
    is.na(P12MOCON) & is.na(P12MOCONO),
  .N
]
```

However, the specified universes in the codebook for `P12MOCON` and `P12MOCONO` are at least accurate.
...
It appears as though the 10 leftovers all did not have intercourse in the past 12 months according to `SEXSTAT`. This is confirmed by 8 of the 10 by `SEX12MO`, but is contradicted by `SEX12MO` for the other 2 cases. These logical contradictions should be resolved.

One issue is that hitherto the century-month values used to determine contraceptive use in past 12 months are not checked for escape values.

```{r, dependson='process-male-data'}
male_2017_2019_data[
  anyuse12m_count > 0 & SEX12MO == 'NO, DID NOT HAVE INTERCOURSE',
  .(
    CWPALLBC01,
    CMLSTYR,
    CMLSXCWP,
...
Some of the contradiction between `SEX12MO` and `anyuse12m_count` involves either a 9998 or 9999 escape value being present or a century-month of intercourse being equal to `CMLSTYR`.

The other issue is that the universe for the `CWPALLBC` series of variables appears to include those with `CMLSXCWP` or `CMFSXCWP` greater than _or equal to_ `CMLSTYR`, whereas elsewhere this comparison is _strictly greater than_.

```{r, dependson='process-male-data'}
male_2017_2019_data[, lapply(.SD, class), .SDcols = c(
  'CMLSTYR',
  'CMLSXCWP',
  'CMFSXCWP',
  'CMLSXP1',
...
```{r, dependson='process-male-data'}
male_2017_2019$Data[
  SEXSTAT %in% c(0, 3:5, 7) &
    male_2017_2019_data$anyuse12m_count > 0,
  .(
    SEXSTAT,
    SEX12MO,
    CMLSTYR,
...
* BC-8a P12MOCONO
* BC-8b P12MOCON How many times used condom during sex last 12 mons
* BC-9 SEXFREQ # times had sex with female in last 4 weeks
* BC-10 CONFREQ # times used condom when had sex with female in last 4 wks
* CF-4 CONDFREQ Percent of time R and CWP used a condom in last 12 mos
* DG-4 PXCONFRQ  Last 12 mos: percent of time R & P used condom (most recent partner)
* DG-4 PXCONFRQ2 Last 12 mos: % of time R & 2nd-to-last P used condom
* DG-4 PXCONFRQ3 Last 12 mos: % of time R & 3rd-to-last P used condom
