...
data_fem <- bind_rows(
  select(raw_fem_2017_2019, CASEID, SEST, SECU, AGER, HISPRACE2, HISP, RACE),
  select(raw_fem_2015_2017, CASEID, SEST, SECU, AGER, HISPRACE2, HISP, RACE)
) |>
  mutate(
    one = 1,
    age_group = cut(
...
targets17_19 <- tribble(
   ~Target_17_19, ~demographic_category,
...
targets15_17 <- tribble(
   ~Target_15_17, ~demographic_category,
...
demographic_population_totals |>
  ...
  select(demographic_category, Target_15_17, Freq, Target_17_19) |>
  mutate(
    `bracketed?` = (Target_15_17 < Freq & Freq < Target_17_19) |
      (Target_15_17 > Freq & Freq > Target_17_19)
  ) |>
  gt() |>
...
```{r create-post-stratification-targets, dependson="create-demographic-post-stratification-targets"}
...
```{r rake-post-stratifications, dependson=c("data-preparation", "create-post-stratification-targets"), eval=TRUE}
...
```{r save-adjusted-weights, dependson="rake-post-stratifications", eval=TRUE}
post_fem |>
