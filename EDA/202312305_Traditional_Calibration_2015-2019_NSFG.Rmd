...
verify_poststratification <- function(svy_before, svy_after, targets, variable_name) {
  svy_before |>
    ...
    mutate(
      ...
      `Calib_Covers?` = Freq <= Calib_upp & Freq >= Calib_low,
      Diff = Calib - Uncal
    ) |>
    select(
      matches(variable_name), Diff, Uncal, Calib, Freq, Calib_low, Calib_upp, `Calib_Covers?`, Calib_Rel_Bias
    )
}
...
summarize_weights <- function(svy) {
  col_name <- as.character(attr(svy, 'survey_vars')$weights[[1]])
  tbl <- svy$variables
  summarize(
    tbl,
    n = sum(!is.na(get(col_name))),
    missing = sum(is.na(get(col_name))),
    min = min(get(col_name)),
    p01 = quantile(get(col_name), 0.01),
    p05 = quantile(get(col_name), 0.05),
    q1 = quantile(get(col_name), 0.25),
    med = median(get(col_name)),
    q3 = quantile(get(col_name), 0.75),
    p95 = quantile(get(col_name), 0.95),
    p99 = quantile(get(col_name), 0.99),
    max = max(get(col_name)),
    mean = mean(get(col_name)),
    sd = sd(get(col_name)),
    deffK = deffK(get(col_name))
  ) |>
  pivot_longer(
    where(is.numeric),
    names_to = 'statistic',
