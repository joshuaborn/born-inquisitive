...
NSFG_2015_2019_for_calibration <- NSFG_2015_2019_fem_data |>
  ...
  left_join(
    NSFG_2015_2019_preg_data |>
      group_by(CASEID) |>
      summarize(
        birth_in_2014 = factor(if_else(
          any(OUTCOME == 'LIVE BIRTH' & Year == 2014),
          'Yes',
          'No'
        )),
        birth_in_2013 = factor(if_else(
          any(OUTCOME == 'LIVE BIRTH' & Year == 2013),
          'Yes',
          'No'
        )),
        birth_in_2012 = factor(if_else(
          any(OUTCOME == 'LIVE BIRTH' & Year == 2012),
          'Yes',
          'No'
        )),
        birth_in_2011 = factor(if_else(
  ...
  mutate(across(starts_with('birth_in_'), \(x) coalesce(x, 'No')))
```
...
NSFG_2015_2019_fem_svy <- NSFG_2015_2019_for_calibration |>
  as_survey_design(
    strata = SEST,
    ids = SECU,
...
NSFG_2015_2019_fem_cal_svy <- rake(
...
NSFG_2015_2019_preg_cal_svy <- NSFG_2015_2019_preg_data |>
  ...
  left_join(
    tibble(
      CASEID = pull(NSFG_2015_2019_fem_cal_svy, CASEID),
      cal_WGT2015_2019 = weights(NSFG_2015_2019_fem_cal_svy)
    ),
    by = 'CASEID'
  ) |>
  as_survey_design(
...
summarize_weights(
  weights(NSFG_2015_2019_fem_svy),
  'original_weights'
) |> full_join(
  summarize_weights(
    weights(NSFG_2015_2019_fem_cal_svy),
    'calibrated_weights'
  ),
  by = join_by(statistic)
) |> full_join(
  summarize_weights(
    weights(NSFG_2015_2019_fem_svy) -  weights(NSFG_2015_2019_fem_cal_svy),
    'differences',
    useUWE = FALSE
  ),
  by = join_by(statistic)
