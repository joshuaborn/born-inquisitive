...
The pep/charagegroups data set from Census appears to have population totals by needed dimensions for the post-stratification targets of the NSFG. However, it only has population totals by the "Resident population" Census universe, which may not correspond well to the NSFG target population.
...
### Post-stratification Targets, "Household Population" Universe

```{r list-07-2018-household, dependson="process-07-2018-monthly-data"}
processed_07_18 |>
  filter(UNIVERSE_f == "Household population" & as.integer(SEX_f) > 1) |>
  gt() |>
  fmt_number(
    pop,
    decimals = 0
...
### Post-stratification Targets, "Civilian Noninstitutionalized Population" Universe

```{r list-07-2018-civilian-noninstitutionalized, dependson="process-07-2018-monthly-data"}
processed_07_18 |>
  filter(UNIVERSE_f == "Civilian noninstitutionalized population" & as.integer(SEX_f) > 1) |>
  gt() |>
  fmt_number(
    pop,
    decimals = 0
...
### Post-stratification Targets, "Civilian Population" Universe

```{r list-07-2018-civilian, dependson="process-07-2018-monthly-data"}
processed_07_18 |>
  filter(UNIVERSE_f == "Civilian population" & as.integer(SEX_f) > 1) |>
  gt() |>
  fmt_number(
    pop,
    decimals = 0
...
### Post-stratification Targets, "Resident Population" Universe

```{r list-07-2018-resident, dependson="process-07-2018-monthly-data"}
processed_07_18 |>
  filter(UNIVERSE_f == "Resident population" & as.integer(SEX_f) > 1) |>
  gt() |>
  fmt_number(
    pop,
    decimals = 0
...
### Post-stratification Targets, "Resident Population Plus Armed Forces Overseas" Universe

```{r list-07-2018-resident-plus-overseas, dependson="process-07-2018-monthly-data"}
processed_07_18 |>
  filter(UNIVERSE_f == "Resident population plus Armed Forces overseas" & as.integer(SEX_f) > 1) |>
  gt() |>
  fmt_number(
    pop,
    decimals = 0
...
### Universe Comparisons with Table 7 from NSFG 2017-2019 Weighting Design Documetation
...
### Differences Between Universes and NSFG Control Totals
