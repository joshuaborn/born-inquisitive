...
guttmacher_APC_2009 <- guttmacher_APC_national |>
  filter(year == 2009) |>
  select(starts_with('abortions'))

guttmacher_APC_2009_total <- pull(guttmacher_APC_2009, abortionstotal)
...
estimate_abortions_by_domain <- function(svy, variable, label) {
  ...
  svy |>
    filter(
      Year %in% 2010:2010 & OUTCOME == 'INDUCED ABORTION'
    ) |>
