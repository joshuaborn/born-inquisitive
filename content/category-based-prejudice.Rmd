...
Before the particular flaws of category-based prejudice are examined, two larger principles about categories are illustrated by way of a more abstract example. There are two such larger principles:

1. Properties of populations are not properties of individuals.
2. Categorization is arbitrary.

```{r shapes-figure, cache=TRUE, fig.height=7, fig.width=7, fig.cap="A population of circles and squares of various colors and sizes, laid out on a two-dimensional plane."}
y_limit <- 10
x_limit <- 10
square_max <- 1
square_min <- 0.25
colors <- as.factor(1:3)
...
The population of individuals used to illustrate these principles is portrayed in Figure \@ref(fig:shapes-figure). The population consists of `r n` shapes, which have been randomly placed in a two-dimensional plane. Each shape has a randomly selected size and color and was randomly selected to be either a circle or a square.
