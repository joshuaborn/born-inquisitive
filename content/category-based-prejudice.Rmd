...
Before the particular flaws of category-based prejudice are examined, two general principles of categories.

```{r shapes-figure, cache=TRUE, fig.height=7, fig.width=7, fig.cap="Circles and squares of various colors and sizes, laid out on a two-dimensional plane."}
y_limit <- 10
x_limit <- 10
square_max <- 1
square_min <- 0.25
...
## Change this so that there is a trend where squares tend to be more toward the left and circles more toward the right, and where cyan tends to be more to the right and purple more to the left. Then a cyan square will be the example of ambiguous categorization by choice of variable.

## Re-run this until there is a spurious trend on the y access for "large" shapes to be up high and "small" shapes to be down low, depending on the cut point.

generate_obj <- function() {
  square_dim <- runif(1, square_min, square_max)
  x <- runif(1, 0, x_limit - square_dim)
  y <- runif(1, 0, y_limit - square_dim)
  x_high <- x + square_dim
  y_high <- y + square_dim
...
The principles are illustrated by the population in Figure \@ref(fig:shapes-figure), which consists of `r n` individual shapes. Whether each shape is a circle or square was chosen at random, and each shape's color, size, and location in a two-dimensional plane were randomly selected.


### Properties of a Population Are Not Properties of Individuals

```{r table-for-averages}
radii <- sort(dt[shape == 'circle', radius])

n_circles <- length(radii)

mean_radius <- round(mean(radii), 2)

knitr::kable(t(round(radii, 2)))
```

Table 1 lists the radius dimensions for all `r n_circles` circles in Figure \@ref(fig:shapes-figure), sorted from smallest to largest and rounded to the hundredths. The arithmetic mean of all these radii is `r mean_radius`. Thus, the average circle radius is `r mean_radius`. However, note that none of the actual circles in Figure \@ref(fig:shapes-figure) have a radius of `r mean_radius`.[^units-of-measurement]
...
This illustrates the principle that _properties of a population are not properties of individuals._ Descriptive statistics like the arithmetic mean of some variable describe properties of a population. They do not describe any property of an individual, even if that individual is categorized as part of the population in question. While it might be common to use language such as "the average circle has radius `r mean_radius`," it is important to remember that the average circle does not exist. It is an unfortunate figure of speech.

While this may seem obvious when applied to a very visual example such as shapes, it is important to remember whenever thinking about populations of anything. There is no entity named by "the average person." The average person does not exist.

```{r, include=FALSE}
table(dt[shape == 'circle', color]) / n_circles
```

This applies not just to averages, but to any property of a population. For instance, the proportion of circles in Figure \@ref(fig:shapes-figure) that are purple is 22.2%, while 66.7% are cyan, and 11.1% are yellow. There is not a single circle in Figure \@ref(fig:shapes-figure) that is 22.2% purple, 66.7% cyan, and 11.1% yellow. Thus, this principle applies not just to means, but also to proportions. Indeed, it applies to any property of a population.


### Categorization Is Arbitrary

The reason why properties of a population are not properties of individuals is that _categorization is arbitrary._ The act of categorization is the creation of a convention that abstracts away details and simplifies thinking about collections of individuals. There is nothing wrong with this practice. Attempting to think about numerous individuals simultaneously would be complicated without such simplification.

However, it is important to remember that any convention done one way could be also be done a different way. Individuals are only included as members in a population because someone construes them to be, whether this is through the formal creation of rules for what individuals get included or through an ad hoc process in which someone manually categorizes individuals as they are encountered.
...
* by choice of variable (color vs shape)
