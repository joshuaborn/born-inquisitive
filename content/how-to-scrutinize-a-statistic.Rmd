...
Once the origin of a statistic is found, the next two genres of questions to ask about the statistic are both related to what is termed "scope of inference" in statistics courses. Scope of inference is divided into two concerns: whether or not the results of a study generalize to a larger population, and whether or not conclusions can be drawn about cause and effect. The former is a concern about sampling, and the latter is a concern about experimental controls. Both of these concerns stem from the possibility of confounding variables leading to false conclusions, and both concerns are addressed by a form of randomization.


### Sampling

Sometimes a statistic is intended to summarize information the state of affairs in a large population. For instance, a country might be interested in the poverty rate of its citizens, which can constitute a population of millions of individual people. If the statistic is intended to generalize, you should ask questions such as _"What was the population sampled?"_ and _"How was the sampling done?"_
...
This is an all too common mistake in interpreting statistics. A study based on a representative sample of women electrical engineers in the Pacific Northwest of the United States and a study based on a representative sample of women financial analysts in Queensland, Australia are not results about "women." The results of each study are about two _different_ populations, and there should be no surprise if the two studies arrive at different results.
...
When scrutinizing statistics, you are concerned with identifying the actual sampled population. The statistic can only be generalized to this sampled population. You should not be distracted by the target population that should have been sampled or even the population the researchers try to portray their results as generalizing to. The way to determine this sampled population is by paying close attention to how the sampling was done. The sampled population consists of those individuals that were eligible to be selected for the sample.
...
This is avoided in representative samples by using randomized sampling. For instance, suppose 35\% of students at the university work part-time. In a simple random sample in which all of the students at the university have an equal probability of being selected for the sample,[^simple] the proportion of students in the sample who work a part time job will also be near 35\%. This will be the case not just for this one potentially confounding variable, but for all confounding variables. In this way randomization ensures that a sample will be representative of the population.
...
A _control_ in the context of an experiment is important for comparison to determine the effect of the intervention. For example, a group of patients suffering from a disease could simply be given a potential new therapy, and then variables pertaining the effects of the disease and overall health could be measured. However, at the end of the experiment, it would not be clear what the effects of the therapy were. Indeed, for many diseases, some proportion of the population suffering from the illness will get better of their own accord over time.
...
Randomization is important for experiments as it is for observational studies. Again, this is due to the potential effects of confounding variables. In an observational study, a confounding variable can alter the properties of a sample, making it unrepresentative, if the distribution of the confounding variables in the sample differs from its distribution in the population. In experiments, a confounding variable can alter the measured effects of an intervention if the distribution of the confounding variable differs between the intervention and control groups.

For example, suppose there was a new flu therapy that underwent a clinical trial, and the result of the clinical trial found that all indicators of health &ndash; e.g., hospitalization rate, mortality rate, recovery time &ndash; were measurably better in the group of volunteers given the new flu therapy than in the control group. That might seem promising. However, what if it were further learned that the average age in the group given the therapy was 32 and the average age in the control group was 67? In this case, if those who are older are more vulnerable to the effects of the flu, age is a confounding variable. This could easily happen if the experimental assignment was not randomized.

Randomization not only reduces the chances of one confounding variable like age being imbalanced between the intervention and the control groups, but randomization, if done well, balances _all_ potentially confounding variables between the intervention and control groups. Thus, randomized assignment is the most potent way to make conclusions about the cause of a measured effect.
...
Randomization in sampling and randomization in experimental assignment might seem very similar. They are both practices that use randomization to address issues that stem from confounding variables. However, they are necessary for two different kinds of conclusion. Randomization in sampling allows a conclusion to be generalized to a population, and randomization in experimental assignment allows conclusions about the cause of a measured effect.

Furthermore, the consequences of these two kinds of randomization are independent of each other. Thus, a study might have randomized sampling, randomized assignment, both, or neither. The ramifications for a study are sometimes summarized in a table such as Table 1.

Not only are the consequences of the two kinds of randomization different, but the study of randomized sampling and of randomized experimental assignment are different subjects &ndash; written about in different textbooks and taught in different classes.[^randomization]

[^randomization]: A point of confusion can arise because, while randomized sampling and randomized experimental assignment are studied in different contexts, randomized experiments are often analyzed using the same mathematical tools as observational studies. Classical statistical inference was developed using the mathematical model of a population and a sample from the population. Rather than reinvent all of the statistical tools developed over the years, analysis of experiments sometimes simply _invokes_ a population model and reuses the same tools. It does not have to be done this way. See [@rosenberger_randomization_2015] for summaries of inference based on randomization rather than a population model.

The important takeaway for your work of scrutinizing statistics that you encounter, is that there are two ways in which randomization can be involved in a statistic, and you
