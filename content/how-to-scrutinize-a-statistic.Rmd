...
<p class="caption">Table 1: The heights of a fictional population of 30 people.</p>
...
The mean height in this population is `r mean_height` inches or about `r trunc(mean_height / 12)`' `r round(mean_height %% 12)`". For illustration purposes, let us take three different simple random samples of 5 people from this population and measure the heights of everyone in each sample. (In practice, because the heights are already known for the whole population, this would not be necessary.)

<p class="caption">Table 2: The heights of those people in a first simple random sample of size 5.</p>

```{r}
kable(height_sample1)
```

The mean height in the sample described in Table 2 is `r mean_height1` inches.

<p class="caption">Table 3: The heights of those people in a simple random sample of size 5.</p>

```{r}
kable(height_sample2)
```

The mean height in the sample described in Table 3 is `r mean_height2` inches.

<p class="caption">Table 4: The heights of those people in a third simple random sample of size 5.</p>

```{r}
kable(height_sample3)
```

The mean height in the sample described in Table 4 is `r mean_height3` inches.
...
Note that statistical significance is a relatively weak claim. When dealing with effect sizes, it is merely the claim that the true effect size is not is not a null value, to use statistical jargon. Statistical significance thus attributes at least some of an effect size to something other than estimation error. Practical significance is discussed first and in more detail in this article deliberately, because _confusing statistical significance with practical significance is a fallacy._
...
Fortunately, you can evaluate both statistical and practical significance simultaneously when confidence intervals are provided. A rule of thumb you can use is to accomplish this is to evaluate the practical significance of a statistic by using the least favorable extreme of the confidence interval to whatever working hypothesis you are considering.
...
This approach has been used in a lot of academic journals, though there is a movement away from the rejection region. Nakagawa and Cuthill [-@nakagawa_effect_2007], though writing for the field of biology specifically, summarize well the reasons for this move in general.

Perhaps because of the historic prevalence of null hypothesis testing in academic journals, the _p_-value comes up again and again when discussing common flaws of empirical research. The remainder of this section summarizes these flaws.


### Not Sharing Effect Size
...
You should know by now that the obvious question to ask of this is, _"How much more?"_ A few numbers are given, so you might think that the answer to this question is given. However, it is not. The value of _t_ is a test statistic that comes from null hypothesis testing and is actually thus redundant with the _p_-value. However, at least the actual value of _t_ is given in this case. The _p_ value is not explicitly given, though the paper asserts it is less than 0.05.

The _r_ value is likely a correlation coefficient, which _can_ be a measure of effect size, albeit a standardized one. However, the paper does not state what correlation coefficient is used, and regardless, it would be more direct to use an unstandardized statistic for effect size. If the correlation coefficient is on the -1 to 1 scale of a Pearson or Spearman correlation coefficient, then 0.22 reflects rather low correlation. If this is the only indication of practical significance given, then you are left to conclude there is not much practical significance to this difference at all.

In short, despite three numbers being listed, the effect size is not directly given and the only information we have about the effect size leads you to believe it is not practically significant. You are simply not told how much more prevalent the stereotype that Asians are quantitatively gifted prevails in America than in Canada.

This point is not a minor tangent discussed in the paper. The paper's thesis turns on this point, since it is a premise in the interpretation of the experimental results. You might, rightly, start to wonder if the effect size is not directly given because it is indeed not practically significant, and the authors do not want to undermine their own thesis.

The intent of this example is not to pick on any one specific academic paper. This example was chosen because it comes from a paper authored by researchers at Harvard University for the journal _Psychological Science_, which is published by the American Psychological Association, and has been cited thousands of times. It is not from the margins of academic research.
...
### Using _p_-Values Without Statistical Inference
