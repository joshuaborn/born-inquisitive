...
Secondly, for multiple citations that lead to different origins, representing them as if they have the same conclusion is a indication that the sources are being misrepresented. It is unlikely that several studies would arrive at the exact same statistic, since statistical inference involves some amount of sampling or assignment error. In these cases, either the error is not being reported, results are being treated as if they were equivalent when they are not,[^effect] or tactics of misrepresentation are being used.
...
However, if you discover the drug decreases daily mean blood glucose by 5 milligrams per decaliter (mg/dl) on average, its effects are not practically significant, because the long-term health benefits were observed to occur when IDDM patients lowered their daily mean blood glucose from levels around 230 mg/dl to levels around 130 mg/dl.
...
How effect size is quantified depends on the typesof variables involved. Broadly, there are two types of variables that you might encounter: categorical variables and numerical variables.[^variables] Categorical variables are the sort in which observations consist of assignment to one of a number of discrete levels such as _small, medium,_ or _large,_ whereas numerical variables are the sort in which observations consist of assignment to a value on the number line, such as 1.7, 42.4, or 1,852.
...
Similarly, in the example of per capita expenditure on food and beverages for off-premises consumption in the United States, the response variable is numerical because a dollar amount is observed for the expenditure on food and beverages. There are many variables that could be used as explanatory variables. One is which state of the United States the expenditures occur in. This would be a categorical variable with 50 levels: Alabama, Alaska, Arizona, etc.
