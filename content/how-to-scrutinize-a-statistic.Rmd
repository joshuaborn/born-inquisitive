...
Researchers have devised many statistics for summarizing an effect size.[^standardized] Most of these are standardized statistics. Standardized statistics convert raw, observed values in their original units of measurement to unitless ratios that oftentimes have reference values.

[^standardized]: Papers by Nakagawa and Cuthill [-@nakagawa_effect_2007] and by Ferguson [-@ferguson_effect_2009] summarize these statistics.

Standardized statistics for effect size have their uses, such as comparing effect sizes when statistics are on different scales or when dealing with inherently abstract and unitless quantities such as those used by psychologists in personality tests. However, whenever you can, you are better off consulting unstandardized statistics where possible.

The reason for this preference is that, as has been seen, the practical significance of an effect size is ultimately derived from subject-matter knowledge. Expressing an effect size with unstandardized statistics keeps the observations in the original units of measurement and closer to real-world subject-matter domain knowledge.


#### Categorical Explanatory Variables

With a categorical explanatory variable, effect sizes are really summarizing the differences in the distribution of the response variable between two or more populations, one population per level of the explanatory variable.

For a numerical response variable, the most common way to express such an effect size is with a difference in means.
