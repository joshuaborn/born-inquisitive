...
Additionally, there are many ways to measure the distribution of a variable in a population, and different ways might lead to different directions of "more" and "less." Thus, specifically with generic comparisons, when it is not specified which measure is being used and when different measures lead to different directions, it is ambiguous which direction the "more" should go.
...
Because nearly _every_ variable has at least a trivially different distribution between two groups, simply asserting that a mean (or other statistic) is greater for one group than the other is largely uninformative. Given enough precision, no two means will be equal. This is a phenomenon described in a well-known quote by statistician John Tukey.
...
Therefore, quantification might be withheld by deceptive analyses intended to make an explanation seem more plausible than it actually is. Alternatively, it might be withheld by flawed, lazy analyses that do not know any better or do not invest enough time in the analysis.
...
[^variation]: To determine how much of variation one variable is explained by another variable, both variables should be measured in the observational units and techniques as linear models or analysis of variance (ANOVA) should be used. "Observational unit" is jargon for the individuals that compose the groups, such as, for example, an individual person.
