...
For any given variable, it is extremely unlikely that it would have the _exact same_ distribution in two different populations. Thus, between two groups, one group is going to have "more" of something than the other. More properly, when comparing two groups, any variable is going to have a greater mean or other statistic for one group than for the other.

Because _every_ variable is at least trivially different between two groups, simply asserting that a mean or other statistic is greater for one group than the other is largely uninformative. Given enough precision, no two means will be equal. This is a phenomenon described in a well-known quote by statistician John Tukey.
