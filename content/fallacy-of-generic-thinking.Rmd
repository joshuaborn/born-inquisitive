...
Psychological research in this area has gone on to demonstrate empirically that the fallacious use of generic generalizations does exist. For instance, Cimpian, Brandone, and Gelman [-@cimpian_generic_2010] found that test subjects accepted generic generalizations at low levels of prevalence, but interpreted them as if they implied high levels of prevalence. This occurred when generic generalizations were evaluated for truth or falsity closer to "some" quantification, but interpreted as if they were closer to "all" quantification.
...
For any given variable, it is extremely unlikely that a variable would have the _exact same_ distribution in two different populations. Thus, between two groups, it should be expected that one of them is going to have "more" of something than the other. The obvious next question to ask of group differences is thus "how much more?" A first answer to this is a summary of [effect size](/how-to-scrutinize-a-statistic.html#effect-size). While an effect size is a helpful initial summary of a difference between groups, it is just that, an initial summary and not a complete description of the difference.
...
On the other hand, if "A" and "B" represent groups of thousands of people, there is not a single point value that represents the thousands of heights of people in group A or a single point value that represents the thousands of heights of people in group B. In this case, two distributions must be compared.
...
One could calculate and compare the arithmetic means of the heights in the two groups, but arithmetic mean is _just_ a measure of the central tendency of a distribution. There are other measures of central tendency, such as the median or mode, which also have their merits. More importantly, there are other properties of a distribution that are just as relevant, such as the dispersion or spread, the skewness, and to a lesser extent, the kurtosis.

Furthermore, these are just measures of a distribution in isolation, what is actually desired in this case are measures that describe the difference between distributions. There are several ways to approach this, such as probability of superiority or proportion of variance explained, but this is beyond the scope of this article. The relevant point for this discussion is that it is never as simple as group A has "more" of something and group B has "less" of it. Assertions such as these are outside the realm of truth or falsity.

Is is, however, true or false whether a specific statistic is greater for one group than for another, such as whether the mean height of group A is greater than the mean height of group B, or whether the median height of group A is greater than the median height of group B. Assertions about differences in mean or median between two groups _are_ either true or false.

However, even with just these two basic statistics, mean and median, it is possible for the mean of group A to be greater than the mean of group B, but the median of group A to be less than the median of group B. Simplifying the heights example to involve only 30 people instead of thousands of people in each group leads to Table 1.
...
As more and more descriptive statistics are used, there are more and more opportunities for such ambiguities to occur.


_**After writing this draft, it has become apparent that there are two ways in which generic comparisons are generic: they can be generic in ambiguity about which group has "more" of something, e.g., mean vs. median example; they can also be generic in magnitude, once direction of "more" effect has been established. Generic generalizations don't have a direction, and so are generic only in magnitude. This should be made more clear in an introduction, and this section split into subsections discussing the two kinds of genericness.**_

* "Generic comparisons" more of a statistician niche; Tukey quote
* Fallacious use in empirical research (group A has "more" of X than group B, and this explains why group A has "more" of Y than group B)
* Uninformative
* Resistant to falsification
