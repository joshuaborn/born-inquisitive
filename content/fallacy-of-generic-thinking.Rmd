...
Unlike a generic generalization, which is generic in precisely one way, there are two ways in which a generic comparison can be generic. A generic generalization fails to describe the proportion of the population for which its predicate is true. When "swans are white" is asserted, it is not clear if this means that 2% or 50% or 99% of swans are white. Similarly, if it is settled that, by any reasonable measure, one group has "more" of something than the other, a generic comparison leaves unspecified how much "more" one group has of something than the other.

Additionally, there are many ways to measure the distribution of a variable in a population, and different ways might lead to different directions of "more" and "less." Thus, with generic comparisons, because the judgement of "more" or "less" is left vague, it can be ambiguous which direction the "more" should go and which direction the "less" should go. 
...
Because nearly _every_ variable is at least trivially different between two groups, simply asserting that a mean (or other statistic) is greater for one group than the other is largely uninformative. Given enough precision, no two means will be equal. This is a phenomenon described in a well-known quote by statistician John Tukey.
...
Even if the mean incomes of the two groups were found to be \$46,382 and \$46,382, there might be a difference if precision were increased to the cent. Indeed, since a mean is a ratio between a sum and a count, this could continue _ad nauseum_ to an arbitrary level of precision.[^significance]
...
As long as the direction of the generic comparison is correct, however, the generic comparison is resistant to falsification. Like generic generalizations, generic comparisons that are resistant to falsification are also highly uninformative. For instance, asserting that group A makes more money on average than group B is uninformative. It could be that the mean income in group A is \$32,950 versus a mean income in group B of \$28,552. It could, however, be that the mean income of group A is \$157,800 versus a mean income in group B of \$28,552.
...
The answer to the fallacy of generic comparisons is similar to the solution to the fallacy of generic generalizations: all that is needed is quantification. However, quantifying a generic comparison is much more nuanced than quantifying a generic generalization.

When quantifying a generic generalization, typically all that is needed is a proportion, in particular, the proportion of the subject population for which the predicate of th generalization is true. For instance, specifying "72.4\%" in "approximately 72.4% of swans are white" is relatively straightforward.

Quantifying a comparison is rarely as simple as providing a single statistic or estimate thereof. Various ways to provide an initial summary of a comparative difference between groups were given in the discussion of [effect size](/how-to-scrutinize-a-statistic.html#effect-size). While an effect size is a helpful initial summary of a difference between groups, it is just that, an initial summary and not a complete description of the difference.

How to compare a variable between two groups is a deep enough topic to merit its own treatment, but is beyond the scope of this article.
