...
The program assumes there is a SAS library set up called `nsfgdata` that contains a table called `d2002_2003femresp` which contains the SAS data table of female respondents file of the Cycle 6 (2002) of the NSFG, as set up by the example programs on the NSFG web site. The `&weights_path.` macro variable is assumbed to contain the path to the weights file.
...
The below SAS program will print out estimates of the number of pregnancies by pregnancy outcome in the analysis period of the Cycle 6 (2002) of the NSFG. Assumptions are similar to the example program above. An additional detail is that when merging the adjusted weights to the pregnancies file, `CASEID`s that do not correspond to any pregnancies should be discarded, otherwise spurious rows will be introduced.
...
The program assumes the female respondents file of the Cycle 6 (2002) of the NSFG is loaded as a data frame in the `cycle_6_femresp` variable, and that the path to the weights file is stored in the `weights_path` variable.

The program uses the [`srvyr`](http://gdfe.co/srvyr/) library and two [`tidyverse`](https://www.tidyverse.org/) libraries to process input and output from the `survey` library.
...
The below R program will print out estimates of the number of pregnancies by pregnancy outcome in the analysis period of the Cycle 6 (2002) of the NSFG. Assumptions are similar to the example program above.
