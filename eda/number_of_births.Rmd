...
fem_preg_data$HISPANIC <- factor(
  fem_preg_data$HISPANIC,
  labels = c(
    'HISPANIC',
    'NON-HISPANIC'
  )
...
Because of the amount of boilerplate code required, confidence intervals for subpopulations are only calculated using `srvyr` below.
...
svyby(
  ...
  ~RACE,
  svytotal,
  design = subset(live_births_2012_2016_svy, HISPANIC == 'NON-HISPANIC')
)
...
svyby(
  ...
  ~RACE,
  svytotal,
  design = subset(live_births_us_2012_2016_svy, HISPANIC == 'NON-HISPANIC')
)
```
