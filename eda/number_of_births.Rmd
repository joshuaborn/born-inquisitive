...
knitr::kable(
  format(
    round(CI(num_births_stat, df = degf(fem_preg_svy)) / 1000),
    big.mark = ','
  ),
...
knitr::kable(
  format(
    round(CI(num_births_us_stat, df = degf(fem_preg_svy)) / 1000),
    big.mark = ','
  ),
...
* Rewrite to use `confint` function using `degf` to get degrees of freedom
